# 化学术语

以下双语词表整理自 [Obsidian-A-Level-STEM 的化学术语笔记](https://github.com/erning/Obsidian-A-Level-STEM/blob/master/80%20References/Terminology%20-%20Chemistry.md)。英文列用作检索键，中文列记录中国大陆通行用法。词表主要覆盖 CAIE A Level 课程范围，是本 skill 的种子参考，不代表完整或唯一的官方标准。

当前内容同步至上游提交 `4877cc7`（2026-07-30）。

## 目录

- [课程结构术语](#课程结构术语)
- [物理化学](#物理化学)
- [无机化学](#无机化学)
- [有机化学](#有机化学)
- [分析与实验技能](#分析与实验技能)

## 课程结构术语

本节仅保留便于检索的课程层级术语。普通主题标题和含义直观的短语不收录，除非其中包含专业术语。

| English | 中国大陆术语 |
|---|---|
| physical chemistry | 物理化学 |
| inorganic chemistry | 无机化学 |
| organic chemistry | 有机化学 |
| analysis | 分析 |
| practical skills | 实验技能 |
| stoichiometry | 化学计量 |
| chemical energetics | 化学能量学 |
| electrochemistry | 电化学 |
| equilibria | 平衡 |
| reaction kinetics | 反应动力学 |
| chemical periodicity | 化学周期性 |
| transition element | 过渡元素 |
| functional group | 官能团 |
| polymerisation | 聚合 |
| organic synthesis | 有机合成 |
| analytical techniques | 分析技术 |

## 物理化学

| English | 中国大陆术语 |
|---|---|
| particle in the atom | 原子中的粒子 |
| atomic radius | 原子半径 |
| proton | 质子 |
| neutron | 中子 |
| electron | 电子 |
| nucleus | 原子核 |
| shell | 电子层 |
| sub-shell | 亚层 |
| orbital | 轨道 |
| principal quantum number | 主量子数 |
| ground state | 基态 |
| electronic configuration | 电子排布 |
| full electronic configuration | 完整电子排布 |
| shorthand electronic configuration | 简写电子排布 |
| electrons in boxes notation | 轨道框图表示法 |
| free radical | 自由基 |
| atomic number, proton number | 原子序数、质子数 |
| mass number, nucleon number | 质量数、核子数 |
| isotope | 同位素 |
| relative charge | 相对电荷 |
| relative mass | 相对质量 |
| relative isotopic mass | 相对同位素质量 |
| relative atomic mass | 相对原子质量 |
| relative molecular mass | 相对分子质量 |
| relative formula mass | 相对式量 |
| unified atomic mass unit | 统一原子质量单位 |
| mole | 摩尔 |
| Avogadro constant | 阿伏加德罗常量 |
| formula | 化学式 |
| empirical formula | 最简式 |
| molecular formula | 分子式 |
| ionic equation | 离子方程式 |
| spectator ion | 旁观离子 |
| state symbol | 状态符号 |
| anhydrous | 无水的 |
| hydrated | 水合的 |
| water of crystallisation | 结晶水 |
| reacting mass | 反应质量 |
| reacting volume | 反应体积 |
| mole concept | 摩尔概念 |
| percentage yield | 百分产率 |
| limiting reagent | 限量试剂 |
| excess reagent | 过量试剂 |
| stoichiometric relationship | 化学计量关系 |
| electronegativity | 电负性 |
| Pauling electronegativity | 鲍林电负性 |
| ionic bonding | 离子键 |
| covalent bonding | 共价键 |
| metallic bonding | 金属键 |
| coordinate bonding, dative covalent bonding | 配位键、给受共价键 |
| electrostatic attraction | 静电吸引 |
| cation | 阳离子 |
| anion | 阴离子 |
| delocalised electron | 离域电子 |
| shared pair of electrons | 共用电子对 |
| expanded octet | 扩展八隅体 |
| orbital overlap | 轨道重叠 |
| sigma (σ) bond | σ 键 |
| pi (π) bond | π 键 |
| hybridisation | 杂化 |
| sp orbital | sp 杂化轨道 |
| sp2 orbital | sp2 杂化轨道 |
| sp3 orbital | sp3 杂化轨道 |
| bond energy | 键能 |
| bond length | 键长 |
| shape of molecule | 分子构型 |
| bond angle | 键角 |
| VSEPR theory | 价层电子对互斥理论（VSEPR） |
| trigonal planar | 平面三角形 |
| linear | 直线形 |
| tetrahedral | 四面体形 |
| pyramidal | 三角锥形 |
| non-linear | 折线形 |
| octahedral | 八面体形 |
| trigonal bipyramidal | 三角双锥形 |
| intermolecular force | 分子间作用力 |
| hydrogen bonding | 氢键 |
| bond polarity | 键极性 |
| dipole moment | 偶极矩 |
| van der Waals' forces | 范德华力 |
| instantaneous dipole-induced dipole forces | 瞬时偶极-诱导偶极作用力 |
| London dispersion forces | 伦敦色散力 |
| permanent dipole-permanent dipole forces | 永久偶极-永久偶极作用力 |
| dot-and-cross diagram | 点叉图 |
| ideal gas | 理想气体 |
| real gas | 实际气体 |
| gas pressure | 气体压强 |
| ideal gas equation | 理想气体状态方程 |
| simple molecular structure | 简单分子结构 |
| giant ionic structure | 巨型离子结构 |
| giant covalent structure | 巨型共价结构 |
| metallic lattice | 金属晶格 |
| enthalpy change | 焓变 |
| standard enthalpy change | 标准焓变 |
| enthalpy change of reaction | 反应焓变 |
| enthalpy change of formation | 生成焓变 |
| enthalpy change of combustion | 燃烧焓变 |
| enthalpy change of neutralisation | 中和焓变 |
| Hess's law | 赫斯定律 |
| bond energy (energetics) | 键能 |
| redox process | 氧化还原过程 |
| oxidation | 氧化 |
| reduction | 还原 |
| oxidation number, oxidation state | 氧化数、氧化态 |
| oxidising agent | 氧化剂 |
| reducing agent | 还原剂 |
| electron transfer | 电子转移 |
| reversible reaction | 可逆反应 |
| dynamic equilibrium | 动态平衡 |
| equilibrium constant | 平衡常数 |
| Le Chatelier's principle | 勒夏特列原理 |
| Brønsted-Lowry theory | 布朗斯特-劳里理论 |
| acid | 酸 |
| base | 碱 |
| conjugate acid-base pair | 共轭酸碱对 |
| rate of reaction | 反应速率 |
| activation energy | 活化能 |
| Boltzmann distribution | 玻尔兹曼分布 |
| homogeneous catalyst | 均相催化剂 |
| heterogeneous catalyst | 多相催化剂 |
| lattice energy | 晶格能 |
| Born-Haber cycle | 玻恩-哈伯循环 |
| enthalpy change of solution | 溶解焓变 |
| enthalpy change of hydration | 水合焓变 |
| entropy change | 熵变 |
| Gibbs free energy change | 吉布斯自由能变 |
| electrolysis | 电解 |
| electrode | 电极 |
| standard electrode potential | 标准电极电势 |
| cell potential | 电池电势 |
| half-cell | 半电池 |
| weak acid | 弱酸 |
| weak base | 弱碱 |
| acid dissociation constant | 酸解离常数 |
| buffer solution | 缓冲溶液 |
| partition coefficient | 分配系数 |
| rate equation | 速率方程 |
| order of reaction | 反应级数 |
| rate constant | 速率常数 |
| half-life | 半衰期 |

## 无机化学

| English | 中国大陆术语 |
|---|---|
| Periodic Table | 元素周期表 |
| chemical periodicity | 化学周期性 |
| period | 周期 |
| group | 族 |
| Period 3 | 第 3 周期 |
| first ionisation energy | 第一电离能 |
| melting point | 熔点 |
| electrical conductivity | 导电性 |
| oxide | 氧化物 |
| chloride | 氯化物 |
| hydrolysis | 水解 |
| thermal stability | 热稳定性 |
| solubility | 溶解度 |
| carbonate | 碳酸盐 |
| nitrate | 硝酸盐 |
| sulfate | 硫酸盐 |
| hydroxide | 氢氧化物 |
| halogen | 卤素 |
| halide ion | 卤离子 |
| hydrogen halide | 卤化氢 |
| volatility | 挥发性 |
| displacement reaction | 置换反应 |
| disproportionation | 歧化 |
| bleach | 漂白剂 |
| nitrogen | 氮 |
| sulfur | 硫 |
| ammonia | 氨 |
| ammonium ion | 铵根离子 |
| nitric acid | 硝酸 |
| sulfuric acid | 硫酸 |
| sulfur dioxide | 二氧化硫 |
| transition element | 过渡元素 |
| d-block element | d 区元素 |
| d orbital | d 轨道 |
| variable oxidation state | 可变氧化态 |
| complex ion | 配离子 |
| ligand | 配体 |
| coordinate bond | 配位键 |
| coordination number | 配位数 |
| ligand exchange | 配体交换 |
| stability constant | 稳定常数 |
| complex | 配合物 |
| colour of complexes | 配合物颜色 |
| stereoisomerism | 立体异构 |
| cis-trans isomerism | 顺反异构 |
| optical isomerism | 光学异构 |
| bidentate ligand | 双齿配体 |
| multidentate ligand | 多齿配体 |
| transition element catalyst | 过渡元素催化剂 |

## 有机化学

| English | 中国大陆术语 |
|---|---|
| organic compound | 有机化合物 |
| functional group | 官能团 |
| homologous series | 同系物系列 |
| saturated compound | 饱和化合物 |
| unsaturated compound | 不饱和化合物 |
| general formula | 通式 |
| structural formula | 结构式 |
| displayed formula | 展示结构式 |
| skeletal formula | 骨架式 |
| nomenclature | 命名法 |
| isomerism | 异构现象 |
| structural isomerism | 结构异构 |
| stereoisomerism | 立体异构 |
| chain isomerism | 碳链异构 |
| position isomerism | 位置异构 |
| functional group isomerism | 官能团异构 |
| cis-trans isomerism (organic) | 顺反异构 |
| optical isomerism (organic) | 光学异构 |
| chiral centre | 手性中心 |
| enantiomer | 对映异构体 |
| racemic mixture | 外消旋混合物 |
| alkane | 烷烃 |
| alkene | 烯烃 |
| arene | 芳烃 |
| benzene | 苯 |
| hydrocarbon | 烃 |
| halogenoalkane | 卤代烷 |
| halogenoarene | 卤代芳烃 |
| alcohol | 醇 |
| phenol | 酚 |
| aldehyde | 醛 |
| ketone | 酮 |
| carbonyl compound | 羰基化合物 |
| carboxylic acid | 羧酸 |
| ester | 酯 |
| acyl chloride | 酰氯 |
| amine | 胺 |
| primary amine | 伯胺 |
| secondary amine | 仲胺 |
| phenylamine | 苯胺 |
| nitrile | 腈 |
| hydroxynitrile | 羟基腈 |
| amide | 酰胺 |
| amino acid | 氨基酸 |
| zwitterion | 两性离子 |
| isoelectric point | 等电点 |
| peptide bond | 肽键 |
| electrophoresis | 电泳 |
| free-radical substitution | 自由基取代 |
| electrophilic addition | 亲电加成 |
| nucleophilic substitution | 亲核取代 |
| electrophilic substitution | 亲电取代 |
| addition-elimination mechanism | 加成-消除机理 |
| nucleophile | 亲核试剂 |
| electrophile | 亲电试剂 |
| carbocation | 碳正离子 |
| hydrolysis (organic) | 水解 |
| oxidation (organic) | 氧化 |
| reduction (organic) | 还原 |
| dehydration | 脱水 |
| esterification | 酯化 |
| condensation reaction | 缩合反应 |
| azo compound | 偶氮化合物 |
| azo group | 偶氮基 |
| diazonium salt | 重氮盐 |
| addition polymerisation | 加聚 |
| condensation polymerisation | 缩聚 |
| monomer | 单体 |
| polymer | 聚合物 |
| repeat unit | 重复单元 |
| polyester | 聚酯 |
| polyamide | 聚酰胺 |
| degradable polymer | 可降解聚合物 |
| biodegradable polymer | 可生物降解聚合物 |
| synthetic route | 合成路线 |
| multi-step synthesis | 多步合成 |
| by-product | 副产物 |

## 分析与实验技能

| English | 中国大陆术语 |
|---|---|
| infrared spectroscopy | 红外光谱法 |
| mass spectrometry | 质谱法 |
| mass spectrum | 质谱图 |
| molecular ion peak | 分子离子峰 |
| fragmentation | 裂解 |
| fingerprint region | 指纹区 |
| thin-layer chromatography | 薄层色谱法 |
| gas / liquid chromatography | 气液色谱法 |
| chromatography | 色谱法 |
| chromatogram | 色谱图 |
| stationary phase | 固定相 |
| mobile phase | 流动相 |
| Rf value | Rf 值 |
| solvent front | 溶剂前沿 |
| baseline | 基线 |
| retention time | 保留时间 |
| carbon-13 NMR spectroscopy | 碳-13 核磁共振谱法 |
| proton (1H) NMR spectroscopy | 质子（1H）核磁共振谱法 |
| NMR spectrum | 核磁共振谱 |
| chemical shift | 化学位移 |
| peak area | 峰面积 |
| splitting pattern | 裂分模式 |
| n + 1 rule | n + 1 规则 |
| singlet | 单峰 |
| doublet | 双峰 |
| triplet | 三重峰 |
| quartet | 四重峰 |
| multiplet | 多重峰 |
| tetramethylsilane (TMS) | 四甲基硅烷（TMS） |
| deuterated solvent | 氘代溶剂 |
| proton exchange | 质子交换 |
| manipulation, measurement and observation | 操作、测量与观察 |
| presentation of data and observations | 数据与观察结果呈现 |
| analysis, conclusions and evaluation | 分析、结论与评价 |
| planning | 规划 |
| defining the problem | 界定问题 |
| method | 方法 |
| dealing with data | 处理数据 |
| conclusion | 结论 |
| evaluation | 评价 |
| apparatus | 仪器 |
| pipette | 移液管 |
| burette | 滴定管 |
| measuring cylinder | 量筒 |
| thermometer | 温度计 |
| titre | 滴定读数 |
| solubility observation | 溶解性观察 |
| table of results | 结果表 |
| line of best fit | 最佳拟合线 |
| anomalous point | 异常点 |
| percentage error | 百分误差 |
| independent variable | 自变量 |
| dependent variable | 因变量 |
| controlled variable | 控制变量 |
| hypothesis | 假设 |
| validity | 有效性 |
| reliability | 可靠性 |
| source of error | 误差来源 |
| experimental procedure | 实验步骤 |
